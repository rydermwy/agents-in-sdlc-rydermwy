...
def create_game() -> tuple[Response, int]:
    try:
        ...
        # Return created game with relationships using base query for consistency
        created_game_query = get_games_base_query().filter(Game.id == new_game.id).first()
        return jsonify(created_game_query.to_dict()), 201
...
def update_game(id: int) -> tuple[Response, int]:
    try:
        ...
        # Return updated game with relationships
        updated_game_query = get_games_base_query().filter(Game.id == id).first()
        return jsonify(updated_game_query.to_dict()), 200
